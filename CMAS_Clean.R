...
day_file_name <- paste0("data/",today(),"-msgfile.csv")
...
flatten_fips <- function(msg) {
  ...
  #seperate out alerts with full state areas, convert directly to fips
  ...
  fips_msg <- left_join(areas, fips_lookup) %>%
          select(msg_id, fips) %>%
          rbind(areas_states)
...
alert_tally[is.na(alert_tally)] <- 0
